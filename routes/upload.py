...
def get_model():
    ...
    Lazy loading — only loads when first upload happens.
    """
...
def upload_document():
    ...
    try:
        ...
        if ocr_result["boxes"]:
            ...
            # No boxes available — use simple regex fallback
        ...
        # Extract just values for postprocessing
        raw_values = {
        ...
        for field_name, field_value in cleaned_values.items():
            # Get confidence from LayoutLMv3 output
...
# Route to serve uploaded document images
